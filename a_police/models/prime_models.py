...
class PrimeStudent(TimeRemarkChoiceBase):
    ...
    exam = models.CharField(max_length=2, choices=exam_choices, default='프모')
    ...
    class Meta:
        ...
        db_table = 'a_police_prime_student'
...
class PrimeRegisteredStudent(TimeRecordField):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='prime_registered_students')
    student = models.ForeignKey(PrimeStudent, on_delete=models.CASCADE, related_name='registered_students')

    class Meta:
        verbose_name = "프라임모의고사 수험정보 연결"
        verbose_name_plural = "프라임모의고사 수험정보 연결"
        unique_together = ['user', 'student']
        db_table = 'a_police_prime_registered_student'

    def __str__(self):
        return f'[Police]PrimeRegisteredStudent:{self.student.reference}({self.student.student_info})'


class PrimeAnswerRecord(TimeRemarkChoiceBase):
    student = models.ForeignKey(PrimeStudent, on_delete=models.CASCADE, related_name='answer_records')
    ...
    class Meta:
        ...
        db_table = 'a_police_prime_answer_record'
...
class PrimeAnswerCount(TimeChoiceBase):
    ...
    class Meta:
        ...
        db_table = 'a_police_prime_answer_count'
