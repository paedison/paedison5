...
from a_predict.utils import get_all_ranks_dict, get_all_score_stat_sub_dict
...
class IndexViewMixIn(
    ...
    def get_properties(self):
        ...
        self.units = self.department_model.objects.filter(exam=self.ex)
        self.departments = self.department_model.objects.filter(exam=self.ex)
    ...
    def get_all_answer_count(self) -> dict:
        ...
        answer_count_list = (
            ...
            .order_by('subject', 'number').values(
                'subject', 'number', 'answer',
    ...
    def get_participant_count(self):
        ...
        queryset = self.answer_count_model.objects.filter(
            exam=self.exam, number=1).values('subject')
